    ...
    async function sendBatch(transactions) {
        ...
        return new Promise((resolve, reject) => {
            ...
            req.on('error', (e) => {
                ...
                resolve({ success: false, message: 'Server unreachable or request timeout', error: e.toString() });
            });

            req.on('timeout', () => {
                req.destroy();
                resolve({ success: false, message: 'Server unreachable or request timeout', error: 'Request timed out' });
            });
